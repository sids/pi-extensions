...
- `bash`: shows a more OpenCode-style running/failed preview, with non-error output dimmed, collapsed output capped at the last 5 visual lines, and a `Ctrl+O` hint to see the full output.
- `edit`: shows a richer remote-style diff with diff stats and tinted change rows inside pi's standard tool box, and supports pi's current `edits[]` call shape.
