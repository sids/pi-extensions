...
function stripGitPrefix(filePath: string | undefined): string | null {
	if (!filePath) {
		return null;
	}
	return filePath.replace(/^(?:a|b)\//, "") || null;
}

function mapParsedDiffType(type: ChangeTypes): DiffFileStatus {
	switch (type) {
		case "new":
			return "added";
		case "deleted":
...
function splitGitDiffPatches(diffText: string): string[] {
	return diffText
		.split(/^diff --git /m)
...
function parseDiffIntoFiles(diffText: string): RawDiffFile[] {
	...
	const patches = splitGitDiffPatches(normalized);
	const parsedFiles = parsePatchFiles(normalized, undefined, true).flatMap((patch) => patch.files);
...
export async function buildDiffReviewData(pi: ExtensionAPI, cwd: string, target: DiffTarget): Promise<DiffReviewData> {
	...
	const visibleRawFiles = rawFiles.filter((rawFile) => {
		const candidatePath = stripGitPrefix(rawFile.parsed.name) ?? stripGitPrefix(rawFile.parsed.prevName) ?? "";
