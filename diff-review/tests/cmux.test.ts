...
describe("cmux open command builders", () => {
	test("builds the expected pane command", () => {
		expect(buildNewPaneArgs("workspace:1", "http://127.0.0.1:1234/review/token")).toEqual([
			"new-pane",
			"--type",
			"browser",
			"--direction",
	...
	test("builds the expected surface command", () => {
		expect(buildNewSurfaceArgs("workspace:1", "pane:current", "http://127.0.0.1:1234/review/token")).toEqual([
			"new-surface",
			"--type",
			"browser",
			"--workspace",
